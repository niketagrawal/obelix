...
class Descriptors:
    ...
    def __init__(self, central_atom, path_to_workflow, output_type):
        ...
        if output_type not in self.supported_output_types:
            raise ValueError(
                f"Output type {output_type} not supported. Please choose from {self.supported_output_types}."
            )
    ...
    def _find_bidentate_ligand(elements, coordinates):
        ...
        :return:
        """
        ligand_atoms, bidentate = molecular_graph(
    ...
    def _calculate_c_c_distance_nbd(elements, coordinates, dictionary):
        ...
        dictionary["distance_pi_bond_1_element_1_idx"] = (
            np.where(np.array(coordinates) == coordinates_c1)[0][0] + 1
        )
        dictionary["distance_pi_bond_1_element_2_idx"] = (
            np.where(np.array(coordinates) == coordinates_c2)[0][0] + 1
        )
        ...
        dictionary["distance_pi_bond_2_element_1_idx"] = (
            np.where(np.array(coordinates) == coordinates_c3)[0][0] + 1
        )
        dictionary["distance_pi_bond_2_element_2_idx"] = (
            np.where(np.array(coordinates) == coordinates_c4)[0][0] + 1
        )

        return dictionary

    @staticmethod
    def _calculate_dihedral_angles_nbd_and_metal_donors(
        dictionary,
        metal_idx,
        ...
        elements,
        coordinates,
        central_carbon_nbd_idx,
        hydrogens_bonded_to_carbon_back_nbd_idxs,
    ):
        """
        ...
        # hydrogen 1
        distance_to_min_donor = calculate_distance(
            hydrogen_1_coordinates, bidentate_min_donor_coordinates
        )
        distance_to_max_donor = calculate_distance(
            hydrogen_1_coordinates, bidentate_max_donor_coordinates
        )
        if distance_to_min_donor < distance_to_max_donor:
            ...
            dihedral_angle_1 = calculate_dihedral(
                closest_donor_coordinates,
                metal_coordinates,
                central_carbon_coordinates,
                hydrogen_1_coordinates,
            )
        else:
        ...
        # hydrogen 2
        distance_to_min_donor = calculate_distance(
            hydrogen_2_coordinates, bidentate_min_donor_coordinates
        )
        distance_to_max_donor = calculate_distance(
            hydrogen_2_coordinates, bidentate_max_donor_coordinates
        )
        if distance_to_min_donor < distance_to_max_donor:
            closest_donor_coordinates = bidentate_min_donor_coordinates
            closest_donor_index = bidentate_min_donor_idx
            dihedral_angle_2 = calculate_dihedral(
                closest_donor_coordinates,
                metal_coordinates,
                central_carbon_coordinates,
                hydrogen_2_coordinates,
            )
        else:
            closest_donor_coordinates = bidentate_max_donor_coordinates
            closest_donor_index = bidentate_max_donor_idx
    ...
    def _buried_volume_quadrant_analysis(
        self,
        filename,
        elements,
        coordinates,
        ...
        for quad_index in range(4):
            ...
            dictionary[quadrant_dictionary[quad_index + 1] + "_quad"] = (
                values[quad_index] / 100
            )

        for oct_index in range(8):
            values = list(octants.values())
            dictionary[octant_dictionary[oct_index] + "_octant"] = (
                values[oct_index] / 100
            )
        if plot_steric_map:
            buried_volume.plot_steric_map(
                filename=os.path.join(
                    self.path_to_workflow, filename + "_steric_map.png"
                )
            )
        return dictionary
    ...
    def _merge_descriptor_dfs(self, old_descriptor_df, new_descriptor_df):
        ...
        :return:
        """
        old_descriptor_df = old_descriptor_df.merge(
            new_descriptor_df,
            on=[
                "filename_tud",
    ...
    def set_output_type(self, new_output_type):
        ...
        if new_output_type not in self.supported_output_types:
            raise ValueError(
                f"Output type {new_output_type} not supported. Please choose from {self.supported_output_types}."
            )
        self.output_type = new_output_type

    def _calculate_steric_electronic_desc_morfeus(
        self,
        geom_type,
        solvent,
        dictionary,
        ...
        bidentate_ligand_atoms, bidentate = self._find_bidentate_ligand(
        ...
        bidentate_ligand_atoms_metal_idx = (
            np.where(
                np.array(bidentate_ligand_atoms_coordinates)
                == coordinates[bidentate_ligand_atoms[-1]]
            )[0][0]
            + 1
        )
        ...
        xtb_functional = (
            2  # indicate whether GFN1 or GFN2 is used for electronic descriptors
        )
        if xtb_functional == 1:
        ...
        atomic_charges = (
            xtb.get_charges()
        )  # determine min/max donor based on xTB charge of the donor atoms
        ...
        bidentate_ligand_atoms_max_donor_idx = (
            np.where(
                np.array(bidentate_ligand_atoms_coordinates)
                == coordinates[bidentate_max_donor_idx - 1]
            )[0][0]
            + 1
        )
        bidentate_ligand_atoms_min_donor_idx = (
            np.where(
                np.array(bidentate_ligand_atoms_coordinates)
                == coordinates[bidentate_min_donor_idx - 1]
            )[0][0]
            + 1
        )
        ...
        if metal_adduct.lower() == "nbd":
            ...
            dictionary.update(
                self._calculate_c_c_distance_nbd(elements, coordinates, dictionary)
            )
            ...
            hydrogens_bonded_to_carbon_back_nbd = (
                nbd_complex.get_hydrogens_bonded_to_carbon_back_nbd()
            )
            # if this is not the case, it means that the NBD is not in the correct orientation in the complex
            if (
                carbon_back_nbd_idx is not None
                and hydrogens_bonded_to_carbon_back_nbd is not None
            ):
                print("NBD found at end of xyz file, calculating stuff the easy way")
                dictionary.update(
                    self._calculate_dihedral_angles_nbd_and_metal_donors(
                        dictionary,
                        metal_idx,
                        bidentate_max_donor_idx,
                        bidentate_min_donor_idx,
                ...
                carbon_back_nbd_idx is None
                and hydrogens_bonded_to_carbon_back_nbd is None
            ):
                print(
                    "NBD not found at end of xyz file, calculating stuff the hard way"
                )
                carbon_back_nbd_and_hydrogens_idx = (
                    nbd_complex.find_central_carbon_and_hydrogens_nbd_openbabel()
                )
                if carbon_back_nbd_and_hydrogens_idx is not None:
                    carbon_back_nbd_idx = carbon_back_nbd_and_hydrogens_idx[0]
                    hydrogens_bonded_to_carbon_back_nbd = [
                        carbon_back_nbd_and_hydrogens_idx[1],
                        carbon_back_nbd_and_hydrogens_idx[2],
                    ]
        ...
        dictionary.update(
            self._buried_volume_quadrant_analysis(
                filename,
                bidentate_ligand_atoms_elements,
                bidentate_ligand_atoms_coordinates,
                dictionary,
                bidentate_ligand_atoms_metal_idx,
                z_axis_atom_index,
                xz_plane_atom_indices,
                excluded_atoms,
                plot_steric_map,
            )
        )
        ...
        try:
            bite_angle = BiteAngle(
                coordinates, metal_idx, bidentate_1_idx, bidentate_2_idx
            ).angle  # unit: degrees
        ...
        dictionary[
            f"distance_{self.central_atom}_max_donor_{self.output_type.lower()}"
        ] = calculate_distance(
            coordinates[metal_idx - 1], coordinates[bidentate_max_donor_idx - 1]
        )
        dictionary[
            f"distance_{self.central_atom}_min_donor_{self.output_type.lower()}"
        ] = calculate_distance(
            coordinates[metal_idx - 1], coordinates[bidentate_min_donor_idx - 1]
        )

        # calculate buried volume descriptors on only the bidentate ligand atoms
        bv_metal_center = BuriedVolume(
            bidentate_ligand_atoms_elements,
            bidentate_ligand_atoms_coordinates,
            bidentate_ligand_atoms_metal_idx,
            radius=3.5,
        ...
        dictionary[f"dispersion_p_int_donor_max_{calculation_method}"] = Dispersion(
            elements, coordinates
        ).atom_p_int[bidentate_max_donor_idx]
        dictionary[f"dispersion_p_int_donor_min_{calculation_method}"] = Dispersion(
            elements, coordinates
        ).atom_p_int[bidentate_min_donor_idx]
        # SASA unit: A^2
        ...
        dictionary[f"dipole_{calculation_method}"] = np.sqrt(
            dipole.dot(dipole)
        )  # unit: Debye dipole moment
        ...
        dictionary[f"electrofugality_{calculation_method}"] = xtb.get_global_descriptor(
            variety="electrofugality", corrected=True
        )  # unit: eV
        dictionary[f"nucleofugality_{calculation_method}"] = xtb.get_global_descriptor(
            variety="nucleofugality", corrected=True
        )  # unit: eV
        dictionary[f"nucleophilicity_{calculation_method}"] = xtb.get_global_descriptor(
            variety="nucleophilicity", corrected=True
        )  # unit: eV
        dictionary[f"electrophilicity_{calculation_method}"] = (
            xtb.get_global_descriptor(variety="electrophilicity", corrected=True)
        )  # unit: eV
    ...
    def _calculate_dft_descriptors_from_log(self, dft, dictionary):
        ...
        if successful_dft_optimization:
            ...
            (
                sum_electronic_and_free_energy,
                sum_electronic_and_enthalpy,
                zero_point_correction,
                entropy,
            ) = dft.extract_thermodynamic_descriptors()
            dictionary[f"sum_electronic_and_free_energy_dft"] = (
                sum_electronic_and_free_energy
            )
            ...
            if self.central_atom is not None:
                ...
                (
                    min_donor_metal_orbital_occupation,
                    min_donor_metal_anti_orbital_occupation,
                ) = (
                    dft.calculate_min_donor_metal_orbital_occupation(),
                    dft.calculate_min_donor_metal_anti_orbital_occupation(),
                )
                dictionary[f"orbital_occupation_min_donor_{self.central_atom}_dft"] = (
                    min_donor_metal_orbital_occupation
                )
                dictionary[
                    f"anti_orbital_occupation_min_donor_{self.central_atom}_dft"
                ] = min_donor_metal_anti_orbital_occupation
                (
                    max_donor_metal_orbital_occupation,
                    max_donor_metal_anti_orbital_occupation,
                ) = (
                    dft.calculate_max_donor_metal_orbital_occupation(),
                    dft.calculate_max_donor_metal_anti_orbital_occupation(),
                )
                dictionary[f"orbital_occupation_max_donor_{self.central_atom}_dft"] = (
                    max_donor_metal_orbital_occupation
                )
                dictionary[
                    f"anti_orbital_occupation_max_donor_{self.central_atom}_dft"
                ] = max_donor_metal_anti_orbital_occupation

            # donor with any other element
            (
                min_donor_other_element_index_list,
                min_donor_other_orbital_occupation_list,
            ) = dft.calculate_min_donor_other_orbital_occupation()
            if (
                not min_donor_other_element_index_list is None
                and not min_donor_other_orbital_occupation_list is None
            ):
                for i, (element_and_index, occupation) in enumerate(
                    zip(
                        min_donor_other_element_index_list,
                        min_donor_other_orbital_occupation_list,
                    )
                    ...
                    dictionary[
                        f"orbital_occupation_min_donor_other_atom_{i + 1}_dft"
                    ] = occupation
                    dictionary[
                        f"orbital_occupation_min_donor_other_atom_{i + 1}_element_dft"
                    ] = other_element
                    dictionary[
                        f"orbital_occupation_min_donor_other_atom_{i + 1}_index_dft"
                    ] = other_element_index
            else:
                dictionary[f"orbital_occupation_min_donor_other_atom_1_dft"] = None
                dictionary[f"orbital_occupation_min_donor_other_atom_1_element_dft"] = (
                    None
                )
                dictionary[f"orbital_occupation_min_donor_other_atom_1_index_dft"] = (
                    None
                )

            (
                min_donor_other_element_index_anti_bonding_list,
                min_donor_other_anti_orbital_occupation_list,
            ) = dft.calculate_min_donor_other_anti_orbital_occupation()
            if (
                not min_donor_other_element_index_anti_bonding_list is None
                and not min_donor_other_anti_orbital_occupation_list is None
            ):
                for i, (element_and_index, occupation) in enumerate(
                    zip(
                        min_donor_other_element_index_anti_bonding_list,
                    ...
                    other_element_index = element_and_index[1]
                    dictionary[
                        f"anti_orbital_occupation_min_donor_other_atom_{i + 1}_dft"
                    ] = occupation
                    dictionary[
                        f"anti_orbital_occupation_min_donor_other_atom_{i + 1}_element_dft"
                    ] = other_element
                    dictionary[
                        f"anti_orbital_occupation_min_donor_other_atom_{i + 1}_index_dft"
                    ] = other_element_index
            else:
                dictionary[f"anti_orbital_occupation_min_donor_other_atom_1_dft"] = None
                dictionary[
                    f"anti_orbital_occupation_min_donor_other_atom_1_element_dft"
                ] = None
                dictionary[
                    f"anti_orbital_occupation_min_donor_other_atom_1_index_dft"
                ] = None

            (
                max_donor_other_element_index_list,
                max_donor_other_orbital_occupation_list,
            ) = dft.calculate_max_donor_other_orbital_occupation()
            if (
                not max_donor_other_element_index_list is None
                and not max_donor_other_orbital_occupation_list is None
            ):
                for i, (element_and_index, occupation) in enumerate(
                    zip(
                        max_donor_other_element_index_list,
                    ...
                    other_element_index = element_and_index[1]
                    dictionary[
                        f"orbital_occupation_max_donor_other_atom_{i + 1}_dft"
                    ] = occupation
                    dictionary[
                        f"orbital_occupation_max_donor_other_atom_{i + 1}_element_dft"
                    ] = other_element
                    dictionary[
                        f"orbital_occupation_max_donor_other_atom_{i + 1}_index_dft"
                    ] = other_element_index
            else:
                dictionary[f"orbital_occupation_max_donor_other_atom_1_dft"] = None
                dictionary[f"orbital_occupation_max_donor_other_atom_1_element_dft"] = (
                    None
                )
                dictionary[f"orbital_occupation_max_donor_other_atom_1_index_dft"] = (
                    None
                )

            (
                max_donor_other_element_index_anti_bonding_list,
                max_donor_other_anti_orbital_occupation_list,
            ) = dft.calculate_max_donor_other_anti_orbital_occupation()
            if (
                not max_donor_other_element_index_anti_bonding_list is None
                and not max_donor_other_anti_orbital_occupation_list is None
            ):
                for i, (element_and_index, occupation) in enumerate(
                    zip(
                        max_donor_other_element_index_anti_bonding_list,
                    ...
                    other_element_index = element_and_index[1]
                    dictionary[
                        f"anti_orbital_occupation_max_donor_other_atom_{i + 1}_dft"
                    ] = occupation
                    dictionary[
                        f"anti_orbital_occupation_max_donor_other_atom_{i + 1}_element_dft"
                    ] = other_element
                    dictionary[
                        f"anti_orbital_occupation_max_donor_other_atom_{i + 1}_index_dft"
                    ] = other_element_index
            else:
                dictionary[f"anti_orbital_occupation_max_donor_other_atom_1_dft"] = None
                dictionary[
                    f"anti_orbital_occupation_max_donor_other_atom_1_element_dft"
                ] = None
                dictionary[
                    f"anti_orbital_occupation_max_donor_other_atom_1_index_dft"
                ] = None
            ...
            # lone pair occupancy
            lone_pair_occupancy_min_donor, lone_pair_occupancy_max_donor = (
                dft.calculate_donor_lone_pair_occupancy()
            )
            dictionary["lone_pair_occupancy_min_donor_dft"] = (
                lone_pair_occupancy_min_donor
            )
            dictionary["lone_pair_occupancy_max_donor_dft"] = (
                lone_pair_occupancy_max_donor
            )
            ...
            # NBO charges
            metal_nbo_charge, min_donor_nbo_charge, max_donor_nbo_charge = (
                dft.calculate_natural_charges()
            )
            if self.central_atom is not None:
                # this will be skipped for the free ligand since there is no metal center there
            ...
            # mulliken charges
            (
                metal_mulliken_charge,
                min_donor_mulliken_charge,
                max_donor_mulliken_charge,
            ) = dft.calculate_mulliken_charges()
            if self.central_atom is not None:
                # this will be skipped for the free ligand since there is no metal center there
                dictionary[f"mulliken_charge_{self.central_atom}_dft"] = (
                    metal_mulliken_charge
                )
            ...
            (
                ...
                electronegativity,
                electrophilicity,
            ) = dft.calculate_electronic_descriptors()
    ...
    def calculate_morfeus_descriptors(
        ...
        if self.output_type.lower() == "xyz":
            ...
            for metal_ligand_complex in tqdm(complexes_to_calc_descriptors):
                ...
                try:
                    (
                        ...
                        bidentate_max_donor_idx,
                        bidentate_min_donor_idx,
                    ) = self._calculate_steric_electronic_desc_morfeus(
                        geom_type=geom_type,
                        solvent=solvent,
                        ...
                        plot_steric_map=plot_steric_map,
                    )
                except:
                ...
                dictionary_for_properties[
                    os.path.basename(os.path.normpath(metal_ligand_complex[:-4]))
                ] = properties
            ...
            if self.descriptor_df is None:
                ...
                self.descriptor_df = self._merge_descriptor_dfs(
                    self.descriptor_df, new_descriptor_df
                )

        elif self.output_type.lower() == "crest":
            complexes_to_calc_descriptors = sorted(
                glob.glob(os.path.join(self.path_to_workflow, "CREST", "*"))
            )
            ...
            for complex in tqdm(complexes_to_calc_descriptors):
                ...
                if ce is not None:
                    ...
                    if (
                        metal_adduct.lower() == "nbd"
                    ):  # in this case there are some additional columns that need to be skipped
                        columns_to_exclude += [
                            "distance_pi_bond_1_element_1",
                            "distance_pi_bond_1_element_2",
                            "distance_pi_bond_1_element_1_idx",
                            "distance_pi_bond_1_element_2_idx",
                            "distance_pi_bond_2_element_1",
                            "distance_pi_bond_2_element_2",
                            "distance_pi_bond_2_element_1_idx",
                            "distance_pi_bond_2_element_2_idx",
                            "dihedral_angle_1_element_1",
                            "dihedral_angle_1_element_2",
                            "dihedral_angle_1_element_3",
                            "dihedral_angle_1_element_4",
                            "dihedral_angle_1_index_1",
                            "dihedral_angle_1_index_2",
                            "dihedral_angle_1_index_3",
                            "dihedral_angle_1_index_4",
                            "dihedral_angle_2_element_1",
                            "dihedral_angle_2_element_2",
                            "dihedral_angle_2_element_3",
                            "dihedral_angle_2_element_4",
                            "dihedral_angle_2_index_1",
                            "dihedral_angle_2_index_2",
                            "dihedral_angle_2_index_3",
                            "dihedral_angle_2_index_4",
                        ]
                    ...
                    try:
                        for conformer_idx, conformer in enumerate(ce.conformers):
                            ...
                            (
                                ...
                                geom_type=geom_type,
                                solvent=solvent,
                                dictionary=conformer.properties,
                                elements=elements,
                                coordinates=coordinates,
                                filename=filename,
                                metal_adduct=metal_adduct,
                            ...
                            for key in [
                                k
                                for k in ce.get_properties().keys()
                                if k in columns_to_exclude
                            ]:
                                if (
                                    conformer.properties[key]
                                    != ce.conformers[0].properties[key]
                                ):
                                    print(
                                        "\n"
                        ...
                        for key in [
                            ...
                            conformer_properties[f"{key}_boltzmann_average"] = (
                                ce.boltzmann_statistic(key)
                            )
                            conformer_properties[f"{key}_boltzmann_std"] = (
                                ce.boltzmann_statistic(key, statistic="std")
                            )
                            conformer_properties[f"{key}_boltzmann_variance"] = (
                                ce.boltzmann_statistic(key, statistic="var")
                            )
                            conformer_properties[f"{key}_Emin_conformer"] = (
                                ce.get_properties()[key][0]
                            )
                    except Exception as e:
                        # if something goes wrong with the molecular graph it usually means that the geometry is wrong
                        print(
                            "Error calculating Morfeus descriptors or Boltzmann averaging for: ",
                            filename,
                        )
                        print(e)
                        print("Make sure to check the geometry")
                ...
                dictionary_for_conformer_properties[
                    os.path.basename(os.path.normpath(complex))
                ] = conformer_properties

            new_descriptor_df = dataframe_from_dictionary(
                dictionary_for_conformer_properties
            )
            if printout:
                print(new_descriptor_df.to_markdown())
            ...
            raise ValueError(
                f"Output type {self.output_type()} not supported. Please choose from {self.supported_output_types}."
            )

    def calculate_dft_descriptors_from_log(
        self,
        geom_type,
        solvent=None,
        extract_xyz_from_log=False,
        printout=False,
        metal_adduct="pristine",
        plot_steric_map=False,
    ):
        ...
        :return:
        """
        supported_metal_adducts = [
            "pristine",
            "acetonitrile",
            "nbd",
        ]  # norbornadiene is placed at bottom of xyz file, so it is a useful pointer for quadrant analysis
        ...
        # get all log files
        complexes_to_calc_descriptors = sorted(
            glob.glob(os.path.join(self.path_to_workflow, "*.log"))
        )
        dictionary_for_properties = {}
        ...
        for metal_ligand_complex in tqdm(complexes_to_calc_descriptors):
            ...
            try:
                ...
                if (
                    not len(coordinates[-1]) == 3
                ):  # if this is true, there is only 1 coordinates array
                    coordinates = coordinates[
                        -1
                    ]  # else morfeus descriptors are calculated for last geometry in log file
                elements = np.array(elements)
            except:
                print(
                    "Error reading elements and coordinates from log file for: ",
                    filename,
                )
                print("Make sure to check the geometry")
            ...
            try:
                dft = DFTExtractor(
                    metal_ligand_complex,
                    metal_idx,
                    bidentate_min_donor_idx,
                    bidentate_max_donor_idx,
                    metal_adduct,
                )
                dft_properties = self._calculate_dft_descriptors_from_log(
                    dft, dft_properties
                )
            except Exception as e:
                # print(e)
            ...
            if extract_xyz_from_log:
                ...
                if elements is not None and coordinates is not None:
                    write_xyz(
                        os.path.join(self.path_to_workflow, xyz_filename),
                        elements,
                        coordinates,
                    )
...
if __name__ == "__main__":
    ...
    descriptors = Descriptors(
        central_atom="Rh",
        path_to_workflow=os.path.join(os.getcwd(), "Workflow"),
        output_type="xyz",
    )
    ...
    dft_descriptors = Descriptors(
        central_atom="Rh",
        path_to_workflow=os.path.join(os.getcwd(), "Workflow"),
        output_type="gaussian",
    )
    dft_descriptors.calculate_dft_descriptors_from_log(
        geom_type="BD",
        solvent=None,
