"""
This tests compares the descriptor values generated by OBeLiX for a given set
of xyz files with the expected descriptor values for the same set of xyz
files. The expected descriptor values are stored in a csv file in the
expected_output folder.

Data used in the test
----------------------

1. The xyz files generated by MACE for the input provided in the
calculate_descriptors fixture.
2. The expected descriptor values for the xyz files generated by MACE.


Test steps
----------

1. The calculate_descriptors fixture is run to generate the input for MACE,
run the MACE workflow, and calculate the descriptors for the xyz files
generated by MACE.
2. The testcases are run with the data provided by the calculate_descriptors
fixture.
3. The output folder generated as a result of the MACE workflow is deleted
...
calculate_descriptors fixture is used to generate the input for MACE, run the
MACE workflow, and calculate the descriptors for the xyz files generated by
MACE.
...
1. The calculate_descriptors fixture is called with the input provided in the
fixture.
2. The input for MACE is generated and the MACE workflow is run to generate
the xyz files.
3. The descriptors are calculated for the xyz files generated by MACE.
4. The descriptor values are written to a csv file in the Descriptors folder.
5. The testcases are run with the data provided by the calculate_descriptors
fixture.
6. The output folder generated as a result of the MACE workflow is deleted
after the test is executed.
...
Execute `pytest -v tests/test_descriptor_calculator/
test_mace_descriptor_workflow/test_mace_descriptor_workflow.py` in the
terminal from the root directory of the repository.
